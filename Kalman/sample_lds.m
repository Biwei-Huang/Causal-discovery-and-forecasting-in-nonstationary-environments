function [x,y] = sample_lds(F, H, Q, R, init_state, init_var, T, models, G, u)
...
state_noise_samples{1}(:,1) = sample_gaussian(zeros(length(Q{1}),1), init_var, 1)';
x = zeros(ss, T);
y = zeros(os, T);

m = models(1);
x(:,1) = init_state(:,m) + state_noise_samples{m}(:,1);
